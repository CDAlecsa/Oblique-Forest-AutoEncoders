...
opts_simulations = {
    'n_runs': 10,
    'max_features_list': np.round(np.arange(0.3, 1.0, 0.05), 2).astype(float),
}
...
#                               Function
###################################################################################################
def run_tabular_max_features_vs_mse_diabetes(folder, folder_plots, filename):
    ...
    for i in opts_simulations["max_features_list"]:
        opts_model["max_features"] = i
        ...
        for count_run in iterator:
            test_params = run_tabular_model(dataset, opts, n_test = opts_diabetes['N_test'])
        ...
        to_print_in_json.append({'max_features': opts_model["max_features"], 'N_test': opts_diabetes['N_test']})


    to_json(folder, filename + "--settings", to_print_in_json)

    title_list = ["max_features", "mse"]
    plot_results_tabular_misc(opts_simulations["max_features_list"], mean_results, std_results, title_list, 
...
path_results = general_path + "tabular_max_features_vs_mse_diabetes/"
path_name_plots = "plots/" 

filename_results = "tabular_max_features_vs_mse_diabetes"

makedir(path_results, True)
makedir(path_results + path_name_plots, False)

run_tabular_max_features_vs_mse_diabetes(path_results, path_name_plots, filename_results)
