...
opts_simulations = {
    'n_runs': 10,
    'max_samples_list': np.round(np.arange(0.2, 1.0, 0.05), 2).astype(float),
}
...
#                               Function
###################################################################################################
def run_tabular_max_samples_vs_mse_diabetes(folder, folder_plots, filename):
    ...
    for i in opts_simulations["max_samples_list"]:
        opts_model["max_samples"] = i
        ...
        to_print_in_json.append({'max_samples': opts_model["max_samples"], 'N_test': opts_diabetes['N_test']})


    to_json(folder, filename + "--settings", to_print_in_json)

    title_list = ["max_samples", "mse"]
    plot_results_tabular_misc(opts_simulations["max_samples_list"], mean_results, std_results, title_list, 
...
path_results = general_path + "tabular_max_samples_vs_mse_diabetes/"
path_name_plots = "plots/" 

filename_results = "tabular_max_samples_vs_mse_diabetes"

makedir(path_results, True)
makedir(path_results + path_name_plots, False)

run_tabular_max_samples_vs_mse_diabetes(path_results, path_name_plots, filename_results)
