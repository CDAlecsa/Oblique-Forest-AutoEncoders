...
opts_simulations = {
    'n_runs': 10,
    'max_depth_list': list(range(3, 30, 3)),
}
...
#                               Function
###################################################################################################
def run_tabular_max_depth_vs_mse_diabetes(folder, folder_plots, filename):
    ...
    for i in opts_simulations["max_depth_list"]:
        opts_estimator["max_depth"] = i
        ...
        to_print_in_json.append({'max_depth': opts_estimator["max_depth"], 'N_test': opts_diabetes['N_test']})


    to_json(folder, filename + "--settings", to_print_in_json)

    title_list = ["max_depth", "mse"]
    plot_results_tabular_misc(opts_simulations["max_depth_list"], mean_results, std_results, title_list, 
...
path_results = general_path + "tabular_max_depth_vs_mse_diabetes/"
path_name_plots = "plots/" 

filename_results = "tabular_max_depth_vs_mse_diabetes"

makedir(path_results, True)
makedir(path_results + path_name_plots, False)

run_tabular_max_depth_vs_mse_diabetes(path_results, path_name_plots, filename_results)
